...
func (c *Config) BuildConfig() (*server.Config, error) {
	...
	if c.HandlerFile != "" {
		...
		// Reads the handler configuration file
		file, err := ioutil.ReadFile(c.HandlerFile)
		if err != nil {
			return nil, fmt.Errorf("failed to read handler configuration file content: %v", err)
		}

		err = json.Unmarshal(file, handlers)
		if err != nil {
			return nil, fmt.Errorf("failed to read handler configuration: %v", err)
		}

		// So that it can be used while creating the JSON-RPC server
		conf.HandlersConfig = handlers
	}
...
func (c *Config) mergeConfigWith(otherConfig *Config) error {
	...
	if otherConfig.HandlerFile != "" {
		c.HandlerFile = otherConfig.HandlerFile
	}
